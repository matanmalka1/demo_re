...
const factPath = (factId: string): ApiPath =>
  `/api/v1/facts/${encodeURIComponent(factId)}`;
  ...
  queryOptions({
    ...
    queryFn: async ({ signal }) => {
      const path =
        status === undefined
          ? factsPath
          : (`${factsPath}?status=${status}` as ApiPath);
...
export const factHistoryQueryOptions = queryOptions({
  ...
  queryFn: async ({ signal }) =>
    (
      await apiRequest<FactHistory>(`${factsPath}/history` as ApiPath, {
        signal,
      })
    ).data,
});
...
export const createPendingFact = async (
...
export const captureClaimFact = async (
  ...
  (
    await apiRequest<FactMutation>(`${factsPath}/from-claim` as ApiPath, {
      method: "POST",
      body,
    })
  ).data;
...
export const transitionFact = async (
  ...
  (
    await apiRequest<FactMutation>(
      `${factPath(factId)}/${command}` as ApiPath,
      {
        method: "POST",
        body,
      },
...
export const attachFact = async (
  ...
  (
    await apiRequest<FactAttachment>(
      `${factPath(factId)}/attachments` as ApiPath,
      {
        method: "POST",
        body,
      },
...
export const confirmAndUseFact = async (
  ...
  (
    await apiRequest<ConfirmAndUseFact>(
      `${factPath(factId)}/confirm-and-use` as ApiPath,
      {
        method: "POST",
        body,
      },
